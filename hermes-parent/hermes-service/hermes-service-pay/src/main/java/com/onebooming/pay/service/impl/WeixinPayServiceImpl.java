package com.onebooming.pay.service.impl;
...
public class WeixinPayServiceImpl implements WeixinPayService {
    ...
    public Map createNative(String out_trade_no, String total_fee) {
        try {
            ...
            param.put("nonce_str", WXPayUtil.generateNonceStr());   //利用工具，生成随机字符
            param.put("body", "畅购");                                //订单描述
            param.put("out_trade_no", out_trade_no);                 //商户订单号
            param.put("total_fee", total_fee);                      //交易金额
            param.put("spbill_create_ip", "127.0.0.1");           //终端IP
            param.put("notify_url", notifyurl);                    //回调地址
            param.put("trade_type", "NATIVE");                     //交易类型，二维码形式

            //2、将参数转成xml字符，并携带签名
            String paramXml = WXPayUtil.generateSignedXml(param, partnerkey);

            ///3、执行请求
            HttpClient httpClient = new HttpClient("https://api.mch.weixin.qq.com/pay/unifiedorder");//微信支付服务器的请求的url
            httpClient.setHttps(true); //请求方式是https
            httpClient.setXmlParam(paramXml); //提交参数
            httpClient.post();

            //4、获取参数
            String content = httpClient.getContent();
            Map<String, String> stringMap = WXPayUtil.xmlToMap(content);//返回数据转成Map
